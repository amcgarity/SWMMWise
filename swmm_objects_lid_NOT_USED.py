...
class temperature(swmmobject):
	heading = '[TEMPERATURE]'
	def __init__(self,str):
		self.pnames = ['Data Element','Values']
		pvals = self.parse(str)
		self.name = pvals[0]
		swmmobject.__init__(self,pvals)
...
class lid_usage(swmmobject):
	heading='[LID_USAGE]'
	def __init__(self,str):
		self.pnames=['Subcatchment','Lid Process','Number','Area','Width','InitSat','FromImp','ToPerv','RptFile','DrainTo']
		pvals=self.parse(str)
		self.name = pvals[1]
		swmmobject.__init__(self,pvals)
...
class conduit(swmmobject):
	...
	def __init__(self,str):
		...
		self.name = pvals[0]
		swmmobject.__init__(self,pvals)
class orifice(swmmobject):
	heading = '[ORIFICES]'
	def __init__(self,str):
		self.pnames = ['Name', 'From node', 'To node', 'Type', 'Offset', 'Qcoeff', 'Gated', 'CloseTime']
		pvals = self.parse(str)
		self.name = pvals[0]
		swmmobject.__init__(self,pvals)
class weir(swmmobject):
	heading = '[WEIRS]'
	def __init__(self,str):
		self.pnames = ['Name', 'From node', 'To node', 'Type', 'Offset', 'Qcoeff', 'Gated', 'EndCon', 'EndCoeff', 'Surcharge'] 
		pvals = self.parse(str)
		self.name = pvals[0]
		swmmobject.__init__(self,pvals)
class xsection(swmmobject):
	heading = '[XSECTIONS]'
	def __init__(self,str):
		self.pnames = ['Link', 'Shape', 'Geom1', 'Geom2', 'Geom3', 'Geom4', 'Barrels', 'Culvert']
		pvals = self.parse(str)
		self.name = pvals[0]
		swmmobject.__init__(self,pvals)
...
objlist = [title,option,evaporation,temperature,raingage,subcatchment,subarea,infiltration,lid_controls,\
lid_usage,junction,outfall,conduit,orifice, weir,xsection,timeser,report]
objdict = {}
for obj in objlist:
